...
const StreakCalendar = () => {
  ...
  useEffect(() => {
    const loadContributions = async () => {
      ...
      try {
        ...
        // Calculate streaks
        const { currentStreak, longestStreak, streakDates } =
          calculateStreaks(contribMap);
  ...
  const calculateStreaks = (contribMap) => {
    ...
    if (
      !contribMap[checkDate.toDateString()] ||
      contribMap[checkDate.toDateString()] === 0
    ) {
  ...
  if (loading) {
    return (
      <div className="w-full bg-[#1a1a1e] border border-white/5 rounded-2xl p-4 shadow-2xl font-sans flex items-center justify-center min-h-[280px]">
  ...
  return (
    <div className="w-full bg-[#1a1a1e] border border-white/5 rounded-2xl px-3 py-3 shadow-2xl font-sans shrink-0">
      {/* Header: Streak Stats */}
        ...
        <div className="w-full">
          <div className="flex items-center justify-between w-full gap-3 mb-1">
            <div className="flex items-center gap-3">
              <span
                className={`flex h-2 w-2 rounded-full ${streakData.currentStreak > 0 ? "bg-orange-500 animate-pulse" : "bg-stone-600"}`}
              ></span>
      ...
      <div className="flex justify-between items-center mb-4 px-2">
        ...
        <div className="flex gap-1">
          <button
            ...
            className="p-1 hover:bg-white/9 rounded-lg text-white font-black"
          >
            <ChevronLeft size={16} />
          </button>
          <button
      ...
      <div className="grid grid-cols-7 gap-y-2 mb-2">
        {["S", "M", "T", "W", "T", "F", "S"].map((d) => (
          <div
            key={d}
            className="text-center text-[10px] font-black text-stone-600 mb-2"
          >
            {d}
          </div>
        ))}
        ...
        {days.map((day) => {
          ...
          const isStart =
            day === 1 || !isStreakDay(day - 1) || date.getDay() === 0;
          const isEnd =
            day === days.length || !isStreakDay(day + 1) || date.getDay() === 6;
