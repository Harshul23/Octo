    ...
    return (
      <div className="w-full grid grid-cols-3 gap-2 shrink-0">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="flex flex-col items-center justify-center gap-2 p-3 rounded-xl bg-[#161616] border border-white/5 min-h-[90px]"
          >
  ...
  return (
    <div className="w-full grid grid-cols-3 gap-2 shrink-0">
      {statItems.map((item, index) => (
        <div
          key={index}
          className="flex flex-col items-center gap-2 p-3 rounded-xl bg-[#161616] border border-white/5 hover:border-white/10 transition-all duration-300"
        >
          ...
          <span className="text-[0.7em] text-neutral-400 tracking-wide text-center">
            {item.label}
          </span>
        </div>
      ))}
